...
/*   Updated: 2025/08/13 17:50:47 by nlaporte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_arg_tree *arg_tree, int *valid)
{
	...
	if (arg_tree->mid->type == TOK_PARA_CLOSE)
	{
		consume_token(lst);
		arg_tree->nud_t = prepare(minishell, lst, 0, valid);
	}
	return (0);
}
...
arg_tree.nud->type == TOK_REDIR_IN || arg_tree.nud->type == TOK_WORD || \
		arg_tree.nud->type == TOK_REDIR_HEREDOC)
...
void print_tree(t_tree *tree, int level, char r)
{
	...
	if (tree->redir_out)
	{
		...
		while (tmp)
		{
			printf(" %s fd=%i",(char *)((t_redir_out *)tmp->data)->path, \
			(int)((t_redir_out *)tmp->data)->fd);
			tmp = tmp->next;
		}
		printf("\n");
	}
