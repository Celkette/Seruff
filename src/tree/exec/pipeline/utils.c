...
/*   Updated: 2025/08/19 13:49:50 by cle-rouz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// gestion erreur dup2
void	fork_dup_fd(t_tree *node, int old, int neew)
{
...
void	fork_dup(t_env *minishell, t_tree *node, int last, int i)
{
	if (i == 0 && !node->redir_in)
		fork_dup_fd(node, minishell->exec.fd[1], STDOUT_FILENO);
	else if (i % 2)
	{
		if (last && !node->redir_in)
			return ((void)fork_dup_fd(node, minishell->exec.fd[0], \
			STDIN_FILENO));
		if (!node->redir_in)
			fork_dup_fd(node, minishell->exec.fd[0], STDIN_FILENO);
		if (!node->redir_out && !last)
			fork_dup_fd(node, minishell->exec.fd2[1], STDOUT_FILENO);
	}
	else
	{
		if (last && !node->redir_in)
			return ((void)fork_dup_fd(node, minishell->exec.fd2[0], \
			STDIN_FILENO));
		if (!node->redir_in)
			fork_dup_fd(node, minishell->exec.fd2[0], STDIN_FILENO);
		if (!node->redir_out && !last)
			fork_dup_fd(node, minishell->exec.fd[1], STDOUT_FILENO);
	}
}
