...
/*   Updated: 2025/07/22 15:56:50 by cle-rouz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//@MàJ  des flags qupote dans la structure meta 
// @param s le caractère à vérifier
// @param meta Pointeur vers la structure contenant les flags de quotes
// @return 0 si quote fermée, 1 si quote simple ouverte, 2 si quote double ouverte
int	flag_quote(char c, t_meta *meta)
{
	if (c == '\'' || c == '\"')
	{
		...
		{
			meta->quote_open = !meta->quote_open;//inverser le flag si quote
			meta->last_quote_is = c;
		}
		else if (meta->quote_open == 1 && meta->last_quote_is == c)
		{
			meta->quote_open = !meta->quote_open;//inverser le flag si quote
			meta->last_quote_is = 0;// réinitialiser le type de quote
		}
		...
		if (meta->single_quote != 0) //PAS SURE D"EN AVOIR BESOIN
			return (1);
		if (meta->double_quote != 0)
			return (2);
	}
	return (0);// quotes fermées
}
...
int	flag_pipe(char c, t_meta *meta)
{
	...
	else if (c != '|' && flag_quote(c, meta) == 0 && flag_space(c, meta) == 0) // Si c n'est pas | ni espace ou une tabulation
	{
		meta->pipe = 0;
