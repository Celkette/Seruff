...
/*   Updated: 2025/08/13 11:59:48 by cle-rouz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	export_no_arg(t_env *minishell)
{
	t_list	*lst;
	t_var	*var;
	t_list	*to_free_lst;//AJOUT CELINE
	
	if (!minishell || !minishell->env_list)
	return (-1);
	//lst = minishell->env_list;//AJOUT CELINE
	lst = sort_env_list_alpha(minishell->env_list); //AJOUT CELINE// Trie la liste
	to_free_lst = lst; //AJOUT CELINE// Garde une référence pour libérer la mémoire plus tard
	while (lst)
	{
		var = (t_var *)lst->data;
	...
	free_token_list(to_free_lst); //AJOUT CELINE// Libère la liste triée
	return (0);
}*/
